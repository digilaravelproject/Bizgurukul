<x-guest-layout>
    ...
    <form method="POST" action="{{ route('register.phase1.store') }}" class="mt-10 space-y-6 px-2 md:px-0" x-data="registerForm({
        ...
        <div class="space-y-6">
            {{-- Full Name --}}
            <div>
                <label for="name" class="block text-xs font-bold text-mutedText uppercase tracking-widest mb-2">Full
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    <label class="block text-xs font-bold text-mutedText uppercase tracking-widest mb-2">Email ID <span
                            class="text-red-500">*</span></label>
                    <div class="relative">
                        <input type="hidden" name="email" :value="email">
                        <input id="email_real" type="email" x-show="!showMasked" x-model="email"
                            @blur="if(email.length > 0) showMasked = true"
                            @input.debounce.500ms="validateEmail(email); checkEmailMatch()" :readonly="isLocked"
                            placeholder="name@example.com"
                            class="w-full px-4 py-3.5 bg-navy/20 border border-primary/10 rounded-xl text-mainText placeholder-slate-500 focus:outline-none focus:border-primary focus:ring-4 focus:ring-primary/10 transition-all font-bold text-sm"
                            :class="{'opacity-75 cursor-not-allowed': isLocked}">

                        <input id="email_display" type="text" x-show="showMasked" :value="maskedEmailDisplay"
                            @focus="if(!isLocked) showMasked = false" :readonly="true" placeholder="name@example.com"
                        ...
                        <div class="absolute inset-y-0 right-3 flex items-center" x-show="isLocked">
                            <button type="button"
                                @click="isLocked = false; showMasked = false; $nextTick(() => document.getElementById('email_real').focus())"
                ...
                <div>
                    <label for="email_confirmation"
                        class="block text-xs font-bold text-mutedText uppercase tracking-widest mb-2">Confirm Email
                        <span class="text-red-500">*</span></label>
                    <input id="email_confirmation" type="email" name="email_confirmation" required
                        x-model="confirmEmail" @input.debounce.500ms="checkEmailMatch()"
                    ...
                    <template x-if="email && confirmEmail && !emailMatchError && !emailError">
                        <p
                            class="text-[10px] text-green-500 font-bold mt-1.5 flex items-center bg-green-500/5 py-1 px-2 rounded-lg w-fit">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    <label for="mobile"
                        class="block text-xs font-bold text-mutedText uppercase tracking-widest mb-2">Mobile Number
                        <span class="text-red-500">*</span></label>
                    <div class="flex group">
                        <span
                            class="inline-flex items-center px-4 rounded-l-xl border border-r-0 border-primary/10 bg-navy/40 text-primary text-sm font-black transition-colors group-focus-within:border-primary group-focus-within:bg-primary/10">
                            +91
                        </span>
                        <input id="mobile" type="text" name="mobile" x-model="mobile" required placeholder="9876543210"
                            maxlength="10"
                ...
                <div>
                    <label for="gender"
                        class="block text-xs font-bold text-mutedText uppercase tracking-widest mb-2">Gender <span
                            class="text-red-500">*</span></label>
                    <div class="relative group">
                        <select id="gender" name="gender" required
                            ...
                            <option value="male" class="bg-navy" {{ old('gender') == 'male' ? 'selected' : '' }}>Male
                            </option>
                            <option value="female" class="bg-navy" {{ old('gender') == 'female' ? 'selected' : '' }}>
                                Female</option>
                            <option value="other" class="bg-navy" {{ old('gender') == 'other' ? 'selected' : '' }}>Other
                            </option>
                        </select>
                        <div
                            class="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none text-mutedText group-hover:text-primary transition-colors">
                            <svg class="w-4 h-4 transition-transform duration-300 group-focus-within:rotate-180"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5" x-data="{ showPass: false }">
                <div>
                    <label for="state_id"
                        class="block text-xs font-bold text-mutedText uppercase tracking-widest mb-2">State/Region <span
                            class="text-red-500">*</span></label>
                    <div class="relative group">
                ...
                <div>
                    <label for="password"
                        class="block text-xs font-bold text-mutedText uppercase tracking-widest mb-2">Create Password
                        <span class="text-red-500">*</span></label>
                    <div class="relative">
                        <input id="password" :type="showPass ? 'text' : 'password'" name="password" required
                            placeholder="Min 8 characters" x-model="password"
                            @input.debounce.500ms="checkPasswordStrength()"
                            class="w-full px-4 py-3.5 bg-navy/20 border border-primary/10 rounded-xl text-mainText placeholder-slate-500 focus:outline-none focus:border-primary focus:ring-4 focus:ring-primary/10 transition-all font-bold text-sm"
                            :class="{'ring-1 ring-red-500 border-red-500': passwordError}">
                        <button type="button" @click="showPass = !showPass"
                            class="absolute inset-y-0 right-3 flex items-center text-slate-400 hover:text-primary transition-all">
    ...
    <div class="mt-8 space-y-4 text-center">
        <p class="text-sm text-mutedText font-medium">
            ...
            <a href="{{ route('login') }}"
                class="font-black text-primary hover:text-secondary transition-colors underline decoration-primary/20 underline-offset-4">Log
                In</a>
        </p>
    </div>
