<x-guest-layout>
    ...
    <div class="text-center mb-8">
        <h1 class="text-3xl font-bold text-mainText tracking-tight">Reset Password</h1>
        <p class="text-mutedText text-sm mt-3">Create a new, strong password for your account.</p>
    </div>

    <form method="POST" action="{{ route('password.store') }}" x-data="{ loading: false, showPass: false }" @submit="loading = true" class="space-y-5">
        @csrf
        ...
        <!-- Confirm Password -->
        <div>
            <label for="password_confirmation" class="block text-sm font-medium text-mainText mb-1.5">{{ __('Confirm Password') }}</label>
