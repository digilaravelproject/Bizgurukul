...
@section('content')
    ...
    <div x-data="bundleManager()" class="font-sans text-mainText min-h-screen">

        <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-10 gap-6 animate-fade-in-down">
            <div>
                <h2 class="text-3xl font-extrabold tracking-tight text-mainText">My Bundles</h2>
                <p class="text-sm text-mutedText mt-1 font-medium">
                    Manage your curated content. Total Bundles: <span
                        class="text-primary font-bold">{{ $bundles->total() }}</span>
                </p>
            </div>
        ...
        <div class="mb-10 p-2 bg-surface border border-primary/10 rounded-[1.5rem] shadow-xl shadow-primary/5 grid grid-cols-1 md:grid-cols-12 gap-4 animate-fade-in-up">
            <div class="md:col-span-12 relative group">
                ...
                <input type="text" x-model.debounce.500ms="search" placeholder="Search bundles by title..."
        ...
        <div class="relative min-h-[400px]">
            ...
            {{-- REAL DATA CONTAINER --}}
            <div x-show="!isLoading" id="bundles-container" @click="handlePagination($event)">
    ...
    <script>
        ...
        function confirmDelete(id) {
            Swal.fire({
                title: 'Delete Bundle?',
                text: "The bundle configuration will be removed permanently.",
                icon: 'warning',
