                            ...
                            <div class="pt-1 flex-1">
                                <div class="flex justify-between items-start">
                                    <div>
                                        ...
                                        <p class="text-xs text-indigo-600 font-bold mt-0.5">Unlocked <span class="uppercase tracking-widest">{{ $item->achievement?->short_title ?? 'Unknown Achievement' }}</span></p>
                                    </div>
                                    <span class="text-[10px] font-bold text-slate-400 bg-slate-50 px-2 py-1 rounded-lg">{{ $item->unlocked_at->diffForHumans() }}</span>
                                </div>
                                <p class="text-[10px] text-slate-400 font-medium mt-2 leading-relaxed">Achieved with reward target of <span class="font-bold">₹{{ number_format($item->achievement?->target_amount ?? 0) }}</span></p>
                            </div>
                        </div>
                    @empty
