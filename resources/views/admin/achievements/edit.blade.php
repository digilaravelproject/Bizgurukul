...
@section('content')
    <div class="font-sans text-mainText min-h-screen space-y-8 animate-fade-in">
        ...
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="text-3xl font-extrabold tracking-tight text-mainText">Edit Milestone</h2>
                <p class="text-sm font-bold text-primary">{{ $achievement->title }}</p>
            </div>
        </div>

        <form action="{{ route('admin.achievements.update', $achievement) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
            @csrf
            @method('PUT')
                    ...
                    <div class="bg-surface border border-primary/10 rounded-[2.5rem] p-8 shadow-2xl shadow-primary/5 space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="space-y-2">
                                ...
                                <input type="text" name="short_title" value="{{ old('short_title', $achievement->short_title) }}"
                                       class="w-full h-14 px-6 bg-primary/5 border-none text-mainText placeholder-mutedText/40 text-sm font-bold focus:ring-2 focus:ring-primary/20 focus:outline-none rounded-2xl transition-all"
                        ...
                        <div class="space-y-2">
                            ...
                            <textarea name="reward_description" rows="4" required
                                      ...
                                      placeholder="Describe the reward or the milestone details...">{{ old('reward_description', $achievement->reward_description) }}</textarea>
                        </div>

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="space-y-2">
                                <label class="text-xs font-black text-mainText uppercase tracking-widest pl-1">Target Earning (₹)</label>
                                <input type="number" name="target_amount" value="{{ old('target_amount', $achievement->target_amount) }}" required min="0" step="0.01"
                                       class="w-full h-14 px-6 bg-primary/5 border-none text-mainText placeholder-mutedText/40 text-sm font-bold focus:ring-2 focus:ring-primary/20 focus:outline-none rounded-2xl transition-all"
                                       placeholder="50000">
                            </div>
                            <div class="space-y-2">
                                ...
                                <select name="reward_type" required
                                    ...
                                    <option value="cash" {{ old('reward_type', $achievement->reward_type) == 'cash' ? 'selected' : '' }}>Cash 💰</option>
                                    <option value="gift" {{ old('reward_type', $achievement->reward_type) == 'gift' ? 'selected' : '' }}>Gift 🎁</option>
                                    <option value="trip" {{ old('reward_type', $achievement->reward_type) == 'trip' ? 'selected' : '' }}>Trip ✈️</option>
                                    <option value="gadget" {{ old('reward_type', $achievement->reward_type) == 'gadget' ? 'selected' : '' }}>Gadget 📱</option>
                                    <option value="custom" {{ old('reward_type', $achievement->reward_type) == 'custom' ? 'selected' : '' }}>Custom Reward</option>
                                </select>
                            </div>
                        </div>
                    </div>
                ...
                <div class="space-y-8">
                    <div class="bg-surface border border-primary/10 rounded-[2.5rem] p-8 shadow-2xl shadow-primary/5 space-y-6">
                        <div class="space-y-4">
                            ...
                            <div x-data="{ photoName: null, photoPreview: '{{ $achievement->reward_image_url }}' }" class="space-y-4">
                                ...
                                <div x-show="!photoPreview" @click="$refs.photo.click()"
                                    ...
                                    <p class="text-xs font-black text-primary/40 group-hover:text-primary uppercase tracking-widest">Update Reward Image</p>
                                </div>
                        ...
                        <div class="space-y-4">
                            <div class="space-y-2">
                                ...
                                <input type="number" name="priority" value="{{ old('priority', $achievement->priority) }}" required min="0"
                                       class="w-full h-14 px-6 bg-primary/5 border-none text-mainText placeholder-mutedText/40 text-sm font-bold focus:ring-2 focus:ring-primary/20 focus:outline-none rounded-2xl transition-all"
                                       placeholder="0">
                            </div>
                            ...
                            <div class="flex items-center justify-between p-4 bg-primary/5 rounded-2xl border border-primary/5">
                                ...
                                <label class="relative inline-flex items-center cursor-pointer">
                                    ...
                                    <input type="checkbox" name="status" value="1" class="sr-only peer" {{ old('status', $achievement->status) ? 'checked' : '' }}>
