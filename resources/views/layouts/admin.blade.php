...
<body class="antialiased bg-navy text-mainText" x-data="{ sidebarOpen: false }">
    ...
    <div class="h-screen flex overflow-hidden">
        ...
        <div class="flex-1 flex flex-col min-w-0 bg-navy relative md:ml-64">
            ...
            <header
                ...
                <div class="flex items-center">
                    ...
                    <button @click="sidebarOpen = true"
                        class="md:hidden mr-4 p-2 bg-primary/5 text-primary rounded-xl hover:bg-primary/10 transition">
                    ...
                    <div class="hidden md:block">
                        ...
                        <p class="text-[10px] text-mutedText uppercase tracking-[0.2em] font-bold">Skills Pehle
                            Management</p>
                    </div>
                </div>
                ...
                <div class="flex items-center space-x-4">
                    {{-- Notification --}}
                    {{-- <button
                        class="p-2.5 text-mutedText hover:text-primary rounded-xl bg-navy border border-transparent hover:border-primary/10 transition relative">
                        <span
                            class="absolute top-2 right-2 w-2 h-2 bg-secondary rounded-full border-2 border-customWhite"></span>
                    ...
                    <div class="relative" x-data="{ profileOpen: false }">
                        <button @click="profileOpen = !profileOpen" @click.away="profileOpen = false"
                            ...
                            <div class="hidden md:block text-left leading-tight">
                                <p class="text-xs font-bold text-mainText">{{ Auth::user()->name ?? 'Administrator' }}
                                </p>
                        ...
                        <div x-show="profileOpen" x-cloak x-transition
                            class="absolute right-0 mt-3 w-56 bg-customWhite border border-primary/10 shadow-xl rounded-2xl py-2 z-50">
                            <div class="px-4 py-3 border-b border-navy">
                                <p class="text-xs font-bold text-mainText">{{ Auth::user()->email ??
                                    'admin@skillspehle.com' }}</p>
                            </div>
                            <a href="{{ route('admin.profile.edit') }}"
                                class="block px-4 py-2 text-xs text-mutedText hover:text-primary hover:bg-navy font-bold">Account
                                Settings</a>
                            <form method="POST" action="{{ route('logout') }}">
                                @csrf
                                <button type="submit"
                                    class="w-full text-left px-4 py-2 text-xs text-secondary hover:bg-secondary/5 font-bold">Sign
                                    Out</button>
                            </form>
                        </div>
                    </div>
            ...
            {{-- Footer --}}
            <footer
                class="bg-customWhite border-t border-primary/5 py-4 px-8 text-mutedText text-[10px] font-bold uppercase tracking-widest flex flex-col md:flex-row justify-between items-center">
                <div>&copy; {{ date('Y') }}. All Rights Reserved by Skillspehle.</div>
        ...
        <template x-if="showVerificationToast">
            <div class="fixed bottom-32 right-10 z-[100] bg-primary text-white px-6 py-4 rounded-xl shadow-2xl shadow-primary/30 flex justify-between items-center gap-4 border border-white/10"
                ...
                <a href="{{ route('admin.verifications.index') }}"
                    class="bg-white text-primary px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest hover:bg-navy hover:text-white transition">
                    View Hub
                </a>
