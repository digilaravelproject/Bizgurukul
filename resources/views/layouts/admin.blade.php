    ...
    <div class="min-h-screen flex overflow-hidden">
        ...
        <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
            ...
            <nav class="flex-1 p-4 space-y-2 overflow-y-auto no-scrollbar" x-data="{ lmsOpen: {{ request()->routeIs('admin.courses.*') || request()->routeIs('admin.lessons.*') ? 'true' : 'false' }} }">
                ...
                {{-- Sidebar: Coupon Management --}}
                <a href="{{ route('admin.coupons.index') }}"
                    class="flex items-center p-3 text-sm transition-colors rounded-xl {{ request()->routeIs('admin.coupons.*') ? 'bg-[#0777be]/10 text-[#0777be] font-bold' : 'text-slate-400 hover:text-white' }}">
                    <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
