...
@section('content')
    ...
    <div class="space-y-6 md:space-y-8 font-sans text-mainText" x-data="walletManager()" x-init="init()">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            @if ($dashboardData['available_balance'] > 0)
                <button @click="withdrawModal = true"
                    class="px-6 py-3 brand-gradient text-white rounded-xl shadow-lg hover:shadow-primary/40 transition-all font-black uppercase text-xs tracking-widest active:scale-95 flex items-center gap-2 w-full md:w-auto justify-center">
                    <i class="fas fa-hand-holding-usd"></i> Request Withdrawal
                </button>
            @else
                <button disabled
                    class="px-6 py-3 bg-navy/50 text-mutedText rounded-xl font-black uppercase text-xs tracking-widest flex items-center gap-2 cursor-not-allowed w-full md:w-auto justify-center">
        ...
        {{-- STATS GRID --}}
        <div id="walletStatsGrid">
            @include('student.wallet.partials.stats_grid')
        </div>

        {{-- FILTERS --}}
        <x-admin.table.filter placeholder="Filter by source..." :showExport="false">
        ...
        <div x-data="{ tab: 'history' }">
                ...
                <div class="bg-surface border border-primary/10 rounded-3xl overflow-hidden shadow-xl shadow-primary/5">
                    <div class="overflow-x-auto">
                        <table class="w-full text-left">
                            <thead
                                class="bg-navy/50 text-[10px] md:text-xs uppercase text-mutedText font-bold tracking-widest border-b border-primary/10">
                                <tr>
                                    @if ($dashboardData['available_balance'] > 0)
                                        <th class="px-6 py-4 w-10 text-center">
                                            <input type="checkbox" @change="toggleAll($event)" :checked="isAllSelected"
                                                class="w-4 h-4 rounded border-primary/30 text-primary focus:ring-primary/50 bg-navy cursor-pointer">
                                        </th>
                                    @endif
            ...
            {{-- TAB: WITHDRAWALS --}}
            <div x-show="tab === 'withdrawals'" style="display: none;" x-transition:enter="transition ease-out duration-300"
                x-transition:enter-start="opacity-0 translate-y-2" x-transition:enter-end="opacity-100 translate-y-0">
                <div class="bg-surface border border-primary/10 rounded-3xl overflow-hidden shadow-xl shadow-primary/5">
                    <div class="overflow-x-auto">
                        <table class="w-full text-left">
        ...
        {{-- WITHDRAWAL MODAL --}}
        <div x-show="withdrawModal" class="fixed inset-0 z-50 flex items-center justify-center pointer-events-none"
            style="display: none;" x-cloak>
            <div x-show="withdrawModal" x-transition.opacity
                class="absolute inset-0 bg-navy/80 backdrop-blur-sm pointer-events-auto" @click="withdrawModal = false">
            </div>

            <div x-show="withdrawModal" x-transition.scale x-transition.opacity
                class="bg-surface border border-primary/20 rounded-3xl shadow-2xl p-6 md:p-8 w-full max-w-md mx-4 relative z-10 pointer-events-auto">
                <button @click="withdrawModal = false" class="absolute top-4 right-4 text-mutedText hover:text-mainText"><i
                        class="fas fa-times"></i></button>

                <h3 class="text-xl font-black text-mainText mb-2 uppercase tracking-widest flex items-center gap-2"><i
                        class="fas fa-hand-holding-usd text-primary"></i> Request Payout</h3>
                <p class="text-xs text-mutedText mb-6 font-semibold">Select commissions from the main table and click submit
                    to process them together, or withdraw all available balance.</p>

                <form @submit.prevent="submitWithdrawal()" action="{{ route('student.wallet.withdraw') }}" method="POST">
                    @csrf
                    <template x-for="id in Object.keys(selectedData)" :key="id">
                    ...
                    <div class="bg-navy p-4 rounded-2xl border border-primary/10 mb-6 flex justify-between items-center">
                        ...
                        <span class="text-2xl font-black text-emerald-500"
                            x-text="formatCurrency(selectedTotal)">₹0.00</span>
                    </div>
        ...
        <script>
            function walletManager() {
                return {
                    ...
                    get selectedTotal() {
                        return Object.values(this.selectedData).reduce((sum, amount) => sum + amount, 0);
                    },

                    get isAllSelected() {
                        const checkboxes = document.querySelectorAll('.commission-checkbox');
                        if (checkboxes.length === 0) return false;
                        return Array.from(checkboxes).every(cb => !!this.selectedData[parseInt(cb.value)]);
                    },
                    ...
                    toggleCommission(id, amount, checked) {
                        if (checked) {
                            this.selectedData[id] = parseFloat(amount);
                        } else {
                            delete this.selectedData[id];
                        }
                        // Trigger reactivity for nested object change
                    ...
                    async fetchData(url = null) {
                        ...
                        this.loading = true;
                        try {
                            let targetUrl = new URL(targetUrlRaw.includes('http') ? targetUrlRaw : window.location.origin +
                                targetUrlRaw);
                            ...
                            if (result.status) {
                                if (result.stats_grid) {
                                    document.getElementById('walletStatsGrid').innerHTML = result.stats_grid;
                                }
                    ...
                    async submitWithdrawal() {
                        ...
                        const result = await Swal.fire({
                            ...
                            text: `Are you sure you want to request payout for ${Object.keys(this.selectedData).length} selected commission(s)?`,
                            icon: 'question',
                        ...
                        if (result.isConfirmed) {
                            ...
                            try {
                                let response = await fetch("{{ route('student.wallet.withdraw') }}", {
                                    ...
                                    body: JSON.stringify({
                                        commission_ids: Object.keys(this.selectedData)
                                    })
                                });
