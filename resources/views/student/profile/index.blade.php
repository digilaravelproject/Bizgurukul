                        ...
                        <form @submit.prevent="updateProfile">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div class="col-span-2 md:col-span-1">
                                    ...
                                    <input type="text" x-model="profile.name" disabled
                                        class="w-full rounded-xl bg-navy border-primary/10 text-mainText opacity-60 cursor-not-allowed focus:ring-0 focus:border-primary/10">
                                </div>
                                ...
                                <div>
                                    ...
                                    <input type="text" x-model="profile.mobile"
                    ...
                    <div x-show="activeTab === 'kyc'" x-transition.opacity>
                        ...
                        @if ($user->kyc_status === 'rejected')
                            <div class="bg-red-500/10 border-l-4 border-red-500 p-4 mb-6 rounded-r-xl">
                                <div class="flex">
                                    <div class="flex-shrink-0"><svg class="h-5 w-5 text-red-500" viewBox="0 0 20 20"
                                            fill="currentColor">
                                    ...
                                    <div class="ml-3">
                                        ...
                                        <p class="text-sm text-red-400 mt-1">Admin Note: <span
                        ...
                        @if ($user->kyc_status === 'verified')
                            <div class="text-center py-12 bg-emerald-500/5 rounded-2xl border border-emerald-500/10">
                                <svg class="w-16 h-16 text-emerald-500 mx-auto mb-3" fill="none" stroke="currentColor"
                                    viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                </svg>
                        ...
                        @if ($user->kyc_status === 'pending')
                            <div class="text-center py-12 bg-amber-500/5 rounded-2xl border border-amber-500/10">
                                <svg class="w-16 h-16 text-amber-500 mx-auto mb-3" fill="none" stroke="currentColor"
                                    viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                </svg>
                                <h3 class="text-lg font-bold text-amber-600">Verification Pending</h3>
                                <p class="text-amber-500 text-sm">We are reviewing your documents. This usually takes 24-48
                                    hours.</p>
                            </div>
                        @endif
                        ...
                        <div x-show="'{{ $user->kyc_status }}' == 'not_submitted' || reapplyMode" class="mt-4">
                            <form @submit.prevent="submitKyc">
                                <div class="space-y-4">
                                    <div>
                                        <label class="block text-sm font-bold text-mutedText mb-1">Full Name (As per
                                            PAN)</label>
                                        <input type="text" x-model="kyc.pan_name" required
                                            class="w-full rounded-xl bg-navy border-primary/10 text-mainText focus:ring-primary focus:border-primary">
                                    </div>
                                    <div>
                                        <label class="block text-sm font-bold text-mutedText mb-1">Upload PAN Card /
                                            Document</label>
                                        <div class="flex items-center justify-center w-full">
                                            <label
                                                ...
                                                <div class="flex flex-col items-center justify-center pt-5 pb-6">
                                                    <svg class="w-8 h-8 mb-3 text-mutedText" fill="none"
                                                    ...
                                                    <p class="text-sm text-mutedText"><span
                                                        ...
                                                        drag and drop</p>
                                                    <p class="text-xs text-mutedText/70">SVG, PNG, JPG or PDF (MAX. 2MB)
                                                    </p>
                                                </div>
                    ...
                    <div x-show="activeTab === 'bank'" x-transition.opacity>
                        ...
                        <form @submit.prevent="submitBank">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    <label class="block text-sm font-bold text-mutedText mb-1">Confirm Account
                                        Number</label>
                            ...
                            <div class="mt-8 flex justify-end">
                                <button type="submit" :disabled="isLoading"
                                    class="bg-primary hover:bg-primary/90 text-white px-6 py-2.5 rounded-xl font-bold transition shadow-lg shadow-primary/20">
                                    Save Bank Details
                                </button>
                            </div>
                        </form>
    ...
    <script>
        function studentProfile() {
            return {
                ...
                submitBank() {
                    if (this.bank.account_number !== this.bank.account_number_confirmation) return Swal.fire('Error',
                        'Account numbers do not match', 'error');
