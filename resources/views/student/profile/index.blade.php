        ...
        <div class="md:grid md:grid-cols-4 md:gap-6">
            ...
            <div class="mt-6 md:mt-0 md:col-span-3">
                <div class="bg-customWhite shadow-sm rounded-3xl border border-primary/10 p-6 min-h-[500px]">
                    ...
                    @if ($user->referrer)
                        <div
                            class="mb-8 p-6 bg-navy/5 rounded-[2rem] border border-primary/10 relative overflow-hidden group">
                            ...
                            <div class="flex flex-col md:flex-row md:items-center gap-6">
                                <div class="flex items-center gap-4">
                                    ...
                                    <div>
                                        <p class="text-xs font-black text-mutedText uppercase tracking-widest">Sponsor Name
                                        </p>
                    ...
                    <div x-show="activeTab === 'kyc'" x-transition.opacity>
                        ...
                        <p class="text-sm text-mutedText mb-6">Submit your documents to verify your identity on the
                            platform.</p>
                        ...
                        @if ($user->kyc_status === 'rejected')
                            <div class="bg-red-500/10 border-l-4 border-red-500 p-4 mb-6 rounded-r-xl">
                                <div class="flex">
                                    <div class="flex-shrink-0"><i class="fas fa-exclamation-circle text-red-500"></i>
                                    </div>
                                    <div class="ml-3">
                    ...
                    <div x-show="activeTab === 'bank'" x-transition.opacity>
                        ...
                        <p class="text-sm text-mutedText mb-6">Manage your payout settings. Changes require document proof
                            and admin approval.</p>
                        ...
                        @elseif($dbBankStatus === 'pending' || $hasPendingUpdate)
                            <div class="mb-8 p-6 bg-amber-500/5 rounded-2xl border border-amber-500/10">
                                ...
                                <p class="text-sm text-amber-500">Your details are under review. Verification usually takes
                                    24 hours.</p>
                                @if ($hasPendingUpdate)
                                    <p class="mt-2 text-xs text-amber-400 font-bold italic">* You have a pending update
                                        request.</p>
                                @endif
                            </div>
                        @elseif($currentBank && $dbBankStatus === 'rejected')
                            <div class="mb-8 p-6 bg-red-500/10 rounded-2xl border border-red-500/20">
                                ...
                                <button @click="changeBankMode = true"
                                    class="mt-3 text-red-500 underline text-xs font-bold">Try again with correct
                                    details</button>
                            </div>
                        @endif
                            ...
                            <form @submit.prevent="submitBank">
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div class="col-span-2">
                                        ...
                                        <div class="flex items-center justify-center w-full">
                                            <label
                                                class="flex flex-col items-center justify-center w-full h-24 border-2 border-primary/10 border-dashed rounded-xl cursor-pointer bg-navy/5 hover:bg-navy/10 transition-colors">
                                                <div class="flex items-center gap-3">
                                ...
                                <div class="mt-8 flex justify-end gap-4">
                                    <button type="submit" :disabled="isLoading"
                                        ...
                                        <span
                                            x-text="bankStatus === 'not_submitted' || bankStatus === 'rejected' ? 'Submit for Verification' : 'Update & Request Approval'"></span>
                                    </button>
                                </div>
                            </form>
                        </div>
                        ...
                        @if ($currentBank && ($dbBankStatus === 'verified' || $dbBankStatus === 'pending'))
                            <div x-show="!changeBankMode && bankStatus !== 'not_submitted' && bankStatus !== 'rejected'"
                                ...
                                <div class="bg-navy/5 p-4 rounded-2xl border border-primary/5">
                                    <p class="text-[10px] uppercase font-black tracking-widest text-mutedText mb-1">Account
                                        Number</p>
                                    <p class="font-bold text-mainText">****{{ substr($currentBank->account_number, -4) }}
                                    </p>
                                </div>
                                <div class="bg-navy/5 p-4 rounded-2xl border border-primary/5">
                                    <p class="text-[10px] uppercase font-black tracking-widest text-mutedText mb-1">IFSC
                                    </p>
                                    <p class="font-bold text-mainText">{{ $currentBank->ifsc_code }}</p>
                                </div>
                            </div>
                        @elseif($dbBankStatus === 'not_submitted')
                            <div x-show="!changeBankMode && bankStatus === 'not_submitted'"
                                class="text-center py-10 bg-navy/5 rounded-2xl border border-dashed border-primary/10">
                ...
                submitBank() {
                    ...
                    axios.post("{{ route('student.bank.save') }}", fd)
                        ...
                        .catch(err => {
                            ...
                            if (err.response?.data?.message) msg = typeof err.response.data.message === 'string' ? err
                                .response.data.message : Object.values(err.response.data.message).flat().join(', ');
                            Swal.fire('Error', msg, 'error');
                        })
                        .finally(() => this.isLoading = false);
                },
